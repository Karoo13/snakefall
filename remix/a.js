...
// tile codes
const SPACE = "0".charCodeAt(0);
const WALL = "1".charCodeAt(0);
const SPIKE = "2".charCodeAt(0);
const FRUIT_v0 = "3".charCodeAt(0); // legacy
const EXIT = "4".charCodeAt(0);
const PORTAL = "5".charCodeAt(0);
const PLATFORM = "6".charCodeAt(0); // legacy
const PLATFORMU = "u".charCodeAt(0);
const PLATFORMD = "d".charCodeAt(0);
const PLATFORML = "l".charCodeAt(0);
const PLATFORMR = "r".charCodeAt(0);
const WOODPLATFORM = "w".charCodeAt(0);
const FOAM = "f".charCodeAt(0);
const DIGGABLEDIRT = "t".charCodeAt(0);
const OPENGATE = "o".charCodeAt(0);
const CLOSEDGATE = "c".charCodeAt(0);
const validTileCodes = [SPACE, WALL, SPIKE, FRUIT_v0, EXIT, PORTAL, PLATFORM, PLATFORMU, PLATFORMD, PLATFORML, PLATFORMR, WOODPLATFORM, FOAM, DIGGABLEDIRT, OPENGATE, CLOSEDGATE];
...
function parseLevel(string) {
  ...
  for (let i = 0; i < mapData.length; i++) {
    let tileCode = mapData[i].charCodeAt(0);
...
function newPoisonFruit(location) {
  const fruits = getObjectsOfType(POISONFRUIT);
  fruits.sort(compareId);
  let i = 0;
  for (; i < fruits.length; i++) {
    if (fruits[i].id !== i) break;
...
function paintAtLocation(location, changeLog) {
  if (isDead()){
    ...
    removeAnyObjectAtLocation(location, changeLog);
    const object = newPoisonFruit(location)
    level.objects.push(object);
    changeLog.push([object.type, object.id, serializeObjectState(null), serializeObjectState(object)]);
  } else throw unreachable();
  render();
...
function move(dr, dc) {
  ...
  let occupiedOpenGates = getOccupiedOpenGateLocations();

  if (isCollision()) {
    let newTile = level.map[newLocation];
...
{
  const newOccupiedOpenGates = getOccupiedOpenGateLocations();
  const newlyOccupiedOpenGates = getSetSubtract(newOccupiedOpenGates, oldOccupiedOpenGates);
...
{
  const newOccupiedOpenGates = getOccupiedOpenGateLocations();
  const nowUnoccupiedOpenGates = getSetSubtract(oldOccupiedOpenGates, newOccupiedOpenGates);
  for (let i = 0; i < nowUnoccupiedOpenGates.length; i++) {
...
function checkMovement(pusher, pushedObject, dr, dc, pushedObjects, dyingObjects) {
  ...
  for (let i = 0; i < forwardLocations.length; i++) {
    ...
    const object = findObjectAtLocation(offsetLocation(forwardLocation, -dr, -dc));
...
function render() {
  ...
  function drawObject(object) {
    switch (object.type) {
      ...
      case POISONFRUIT: {
        const poisionRowcol = getRowcol(level, object.locations[0]);
        drawCircle(poisionRowcol.r, poisionRowcol.c, 1, "#570");
        break;
      }
      default: throw unreachable();
    }
  ...
  function drawOneWayWall(fillStyle, r, c, dr, dc, adjacentTiles) {
    const extendTop    = adjacentTiles[0][1] === SPACE || adjacentTiles[0][1] === WOODPLATFORM || adjacentTiles[0][1] === PLATFORMU ? tileSize/15 : 0;
    const extendLeft   = adjacentTiles[1][0] === SPACE ? tileSize/15 : 0;
    const extendBottom = adjacentTiles[2][1] === SPACE ? tileSize/15 : 0;
    const extendRight  = adjacentTiles[1][2] === SPACE ? tileSize/15 : 0;
  ...
  function drawTileOutlines(r, c, isOccupied, outlineThickness) {
    ...
    //const complementPixels = (1 - 2 * outlineThickness) * tileSize;
